...
void drawLines()
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Pionowa linia laczaca 2 poljajka
        glColor3fv(colors[i][n - 1]);
        glVertex3fv(arr[i][n - 1]);

        glColor3fv(colors[(i + 1) % n][n - 1]);
        glVertex3fv(arr[(i + 1) % n][n - 1]);
        // Pozioma linia laczaca 2 poljajka
        glColor3fv(colors[i][n - 1]);
        glVertex3fv(arr[i][n - 1]);
...
void keys(unsigned char key, int x, int y)
{
    ...
    if (key == 't') {
        teapotDraw = true; eggDraw = false;
    }
    if (key == 'e') {
        eggDraw = true; teapotDraw = false;
    }
    RenderScene();
}
...
void MyInit()
{
    ...
    //    pBytes = LoadTGAImage("/Users/lukamitrovic/Downloads/tekstury/P5_t.tga", &ImWidth, &ImHeight, &ImComponents, &ImFormat);
